...
import { TmdbSearchResult, getPosterUrl, STATUS_MAP } from '@/lib/types'
...
import { Search, Film, Tv, Plus, Star, Calendar, Loader2 } from 'lucide-react'
...
export function Discover() {
  ...
  const searchTmdb = useCallback(async (query: string) => {
    ...
    try {
      ...
      toast({ title: '搜索失败', description: '无法连接到TMDB', variant: 'destructive' })
    }
    setLoading(false)
  }, [toast])

  const addMedia = async (item: TmdbSearchResult) => {
    setAdding(true)
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        ...
        <Button onClick={() => searchTmdb(searchQuery)} disabled={loading} className="h-11 px-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-lg font-medium text-muted-foreground">搜索你喜欢的影视作品</p>
          <p className="text-sm text-muted-foreground/70 mt-1">输入中文名或英文名搜索 TMDB 数据库</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
